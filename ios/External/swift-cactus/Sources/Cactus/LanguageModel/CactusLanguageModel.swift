// For iOS, import CXXCactusDarwin directly
...
extension CactusLanguageModel {
  ...
  public func chatCompletion(
    ...
    // Convert ChatMessage to a simple struct ensuring role is String
    struct SimpleMessage: Codable {
      let role: String
      let content: String
      let images: [String]?
    }
    let simpleMessages = messages.map { SimpleMessage(role: $0.role.rawValue, content: $0.content, images: $0.images?.map(\.path)) }

    let result = try withTokenCallback(onToken) { userData, onToken in
      cactus_complete(
        self.model,
        String(decoding: try Self.inferenceEncoder.encode(simpleMessages), as: UTF8.self),
        buffer,
